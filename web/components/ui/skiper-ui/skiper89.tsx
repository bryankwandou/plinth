...
export { Skiper89 };
/** Plinth adaptation of Skiper89: the page scroll ring on its own, without demo content or drag. */
export function ScrollProgressRing({ className, label = "Page progress" }: { className?: string; label?: string }) {
  const { scrollYProgress } = useScroll();
  const [pct, setPct] = useState(0);
  const clamped = useTransform(scrollYProgress, (v) => Math.min(Math.max(v, 0), 1));
  useMotionValueEvent(clamped, "change", (v) => setPct(Math.round(v * 100)));
  const r = 18;
  return (
    <div className={cn("group fixed bottom-5 right-5 z-30", className)} role="progressbar" aria-label={label} aria-valuemin={0} aria-valuemax={100} aria-valuenow={pct}>
      <NumberFlow value={pct} suffix="%" className="absolute -top-1 left-1/2 -translate-x-1/2 -translate-y-full text-xs font-medium tabular-nums text-[var(--mut)] opacity-0 transition-opacity group-hover:opacity-100" />
      <div className="flex size-12 items-center justify-center rounded-2xl border border-[var(--line)] bg-[color-mix(in_srgb,var(--bg)_70%,transparent)] text-[var(--acc)] backdrop-blur">
        <svg className="size-10" viewBox="0 0 48 48" role="presentation">
          <circle cx="24" cy="24" r={r} stroke="currentColor" strokeWidth="3" className="opacity-20" fill="none" />
          <motion.circle cx="24" cy="24" r={r} stroke="currentColor" strokeWidth="3" fill="none" strokeLinecap="round" style={{ pathLength: clamped, rotate: -90, transformOrigin: "50% 50%" }} />
        </svg>
      </div>
    </div>
  );
